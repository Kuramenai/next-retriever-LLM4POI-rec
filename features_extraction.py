...
# write_csv(X_train, out_dir / "step3_train_features.csv")
# write_csv(X_val, out_dir / "step3_validation_features.csv")
# write_csv(X_test, out_dir / "step3_test_features.csv")

# Extract temporal features
# dur_mean, dur_std = fit_duration_normalizer_from_checkins(train_checkins)

# X_train_temp, train_temp_meta = build_temporal_feature_matrix(
#     train_checkins, dur_mean, dur_std
# )
# X_val_temp, val_temp_meta = build_temporal_feature_matrix(
#     val_checkins, dur_mean, dur_std
# )
# X_test_temp, test_temp_meta = build_temporal_feature_matrix(
#     test_checkins, dur_mean, dur_std
# )

# # Save temporal features
# write_csv(X_train_temp, out_dir / "step3_train_temporal_features.csv")
# write_csv(X_val_temp, out_dir / "step3_validation_temporal_features.csv")
# write_csv(X_test_temp, out_dir / "step3_test_temporal_features.csv")
...
# vectorizer = TfidfVectorizer(
#     analyzer="word",
#     ngram_range=(1, 2),  # start with unigrams + bigrams
#     token_pattern=r"(?u)\b\S+\b",  # keep underscore-joined category tokens
# )

# X_train_cat = vectorizer.fit_transform(train_cat_df["category_doc"])
# X_val_cat = vectorizer.transform(val_cat_df["category_doc"])
# X_test_cat = vectorizer.transform(test_cat_df["category_doc"])

# # L2-normalize category block separately, as planned
# X_train_cat = normalize(X_train_cat, norm="l2")
# X_val_cat = normalize(X_val_cat, norm="l2")
# X_test_cat = normalize(X_test_cat, norm="l2")

# # Save category features
# write_csv(X_train_cat, out_dir / "step3_train_category_features.csv")
# write_csv(X_val_cat, out_dir / "step3_validation_category_features.csv")
# write_csv(X_test_cat, out_dir / "step3_test_category_features.csv")
